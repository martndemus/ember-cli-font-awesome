...
function buildCacheKey(addon) {
  let cacheKeyParts = [
    hashForDep(addon.project.root),
...
module.exports = class PruneUnusedIcons extends BroccoliFilter {
  constructor(inputNodes, options) {
    ...
    this.cacheKey = buildCacheKey(options.addon);
    this.persistentCache = new DiskCache(this.cacheKey);
    this.postcss = postcss.plugin('postcss-remove-unused-fa-icons', () => {
      return root => {
        ...
        let usedIcons = this.options.addon.includeStaticIcons;
  ...
  processString(str /*, relativePath */) {
    // let cacheEntry = this.persistentCache.get('font-awesome-used-icons');
    // let usedIconsPerTemplate = cacheEntry.value;
    let usage = this.processUsage();
  ...
  processUsage() {
    ...
    let thing = this.persistentCache.get(this.cacheKey);
    if (thing.isCached) {
      let cachedUsage = JSON.parse(thing.value);
      normalizedUsage = Object.assign(cachedUsage, normalizedUsage)
    }
    this.persistentCache.set(this.cacheKey, JSON.stringify(normalizedUsage));
